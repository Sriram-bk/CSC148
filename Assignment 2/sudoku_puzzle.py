...
class SudokuPuzzle(Puzzle):
    ...
    === Attributes ===
    @type _n : int
        Dimensions of the sudoku puzzle
    @type _symbols: list[str]
        Current configuration of the sudoku puzzle.
    @type _symbol_set : set[str]
        Set of symbols that can be used in the sudoku puzzle.
    """
    ...
    def fail_fast(self):
        ...
        while position < self._n ** 2:
            if self._symbols[position] == '*':
                # Get set of symbols that can't be placed in
                # the empty position.
                used_symbols = set()
                used_symbols.update(self._row_set(position))
                used_symbols.update(self._column_set(position))
                used_symbols.update(self._subsquare_set(position))
                used_symbols.remove('*')

                # It is not possible to complete a sudoku puzzle if there
                # is one open position that has no symbols available to put
                # in it.
                if len(used_symbols) == len(self._symbol_set):
                    return True
            position += 1
